...
import Innertube, { Platform, type Types, type Player } from "youtubei.js";
...
export function createYoutubeFetch(options?: YoutubeOptions) {
	const f: typeof fetch = (
		...
		if (options?.proxy) {
			init ??= {};
			(init as { dispatcher: ProxyAgent }).dispatcher = options.proxy;
		}
...
export async function getInnertube(
	options?: YoutubeOptions & { force?: boolean },
): Promise<Innertube> {
	if (!tube) {
...
export async function decipherLiveStreamUrl(
	url: string,
	player: Player,
	poToken: string,
): Promise<string> {
	...
	for (let i = 0; i + 1 < pathParts.length; i += 2) {
		urlObject.searchParams.set(
			pathParts[i],
			decodeURIComponent(pathParts[i + 1]),
		);
	}
