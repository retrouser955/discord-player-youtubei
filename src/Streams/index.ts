...
const VIDEO_STREAMER_MAP: Record<
	TrialItem,
	(info: Track, ext: YoutubeExtractor) => Promise<Readable>
> = {
	peer: async (info, ext) => {
		if (ext.options.peer?.length > 0) {
			ext.context.player.debug(
				"[YouTube]: Peers detected. Trying peer streaming ...",
			);
			const peer =
				ext.options.peer[Math.floor(Math.random() * ext.options.peer.length)];
			const url = peer.parseUrl(getVideoId(info.url));
			const headers =
				typeof peer.headers === "function"
					? await peer.headers(url)
					: peer.headers;

			ext.context.player.debug(
				`[YouTube]: Attempting to stream from peer { url: ${url} } ...`,
			);
			const stream = downloadPeer(url, headers);
			ext.context.player.debug(
				"[YouTube]: Stream extraction from peer successful.",
			);
	...
	"yt-dlp": async (info, ext) => {
		try {
			ext.context.player.debug(
				"[YouTube]: Attempting to stream with yt-dlp if installed ...",
			);
			...
			if (
				...
				error.type === YTDLPErrorType.NOT_INSTALLED
			) {
				ext.context.player.debug(
					"[YouTube]: yt-dlp is not installed. Skipping.",
				);
			} else throw error;
		}
	},
	adaptive: async (info, ext) => {
		const stream = await createAdaptiveStreamMultiStep(
			ext.innertube,
			getVideoId(info.url),
			ext.options.downloads?.adaptiveStream?.customClientOrder,
		);
		return stream;
	},
	sabr: async (info, ext) => {
		...
		for (const method of options.downloads.trialOrder) {
			...
			try {
				...
				extractor.context.player.debug(
					"[YouTube]: Stream extraction of " +
						createJsonLikeDebug(info) +
						" failed with the method " +
						method,
				);
