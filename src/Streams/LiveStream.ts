...
import { Platform, type Types, Constants, Utils } from "youtubei.js";
...
class SequenceBasedLiveStream extends Readable {
	constructor(
		...
		async function initializeStreamInfo(): Promise<void> {
			const response = await innertube.session.http.fetch_function(
				manifestUrl,
				{
			...
			const baseUrlMatch = xml.match(
				/<(\w+:)?BaseURL>([^<]+)<\/(\w+:)?BaseURL>/,
			);
			if (baseUrlMatch) baseUrl = baseUrlMatch[2];

			const startNumberMatch = xml.match(/startNumber="(\d+)"/);
			const startSeq = startNumberMatch ? parseInt(startNumberMatch[1], 10) : 0;

			const templateMatch = xml.match(
				/<(\w+:)?SegmentTemplate[^>]*media="([^"]+)"/,
			);
			if (templateMatch) segmentTemplate = templateMatch[2];

			const timelineMatch = xml.match(
				/<(\w+:)?SegmentTimeline>([\s\S]*?)<\/(\w+:)?SegmentTimeline>/,
			);
			const listMatch = xml.match(
				/<(\w+:)?SegmentList>([\s\S]*?)<\/(\w+:)?SegmentList>/,
			);
			...
			if (timelineMatch) {
				...
				for (const seg of segments) {
					...
					totalSegments += 1 + (rMatch ? parseInt(rMatch[1], 10) : 0);
				}
			} else if (listMatch) {
				totalSegments = (listMatch[2].match(/<(\w+:)?SegmentURL/g) || [])
					.length;
			}

			currentSequence =
				totalSegments > 0 ? startSeq + totalSegments - 3 : startSeq;

			if (!baseUrl && !segmentTemplate) {
				throw new Error(
					"Manifest parsing failed: No BaseURL and no SegmentTemplate found.",
				);
			}
			...
			if (segmentTemplate) {
				...
				if (numberIdx !== -1) {
					...
					segmentUrlSuffix = resolvedTemplate.substring(
						numberIdx + "$Number$".length,
					);
				} else {
		...
		async function handle403Error(): Promise<void> {
			if (retryCount++ > MAX_LIVESTREAM_RETRY_ATTEMPT)
				throw new Error("Max retries reached");

			const newManifest = await generateManifestUrl(
				dashUrl,
				videoId,
				innertube,
			);
...
export async function createLiveStream(
	...
	try {
		...
		return new SequenceBasedLiveStream(
			manifestUrl,
			videoId,
			dashUrl,
			innertube,
...
async function generateManifestUrl(
	...
	try {
		...
		const url = await decipherLiveStreamUrl(
			dashUrl,
