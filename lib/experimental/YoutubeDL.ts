...
const { YOUTUBE_DL_PATH, YOUTUBE_DL_HOST, YOUTUBE_DL_DIR, YOUTUBE_DL_FILE } = (ytdlExec as unknown as {
...
export class UpdateError extends Error {
    ...
    constructor(type: UpdateErrorType, message?: string, errorOptions?: ErrorOptions) {
...
export async function getDlBinary() {
    ...
    if(!binaryResponse.ok) throw new UpdateError("download", "Could not update youtube-dl");

    if(binaryResponse.headers.get("Content-Type") !== "application/octet-stream") {
        const { assets } = await binaryResponse.json();
        const { browser_download_url } = assets.find(({name}: { name: string }) => name === YOUTUBE_DL_FILE);

        binaryResponse = await fetch(browser_download_url);
        if(!binaryResponse.ok) throw new UpdateError("download", "Could not update youtube-dl");
    }

    if(!binaryResponse.body) throw new UpdateError("body_not_found", "Could not find body of the file")
...
export async function generateStreamWithYoutubeDL(track: Track, youtubei: YoutubeiExtractor) {
