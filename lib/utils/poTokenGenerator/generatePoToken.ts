...
export async function generateTrustedToken(options: GeneratorOptions = {}) {
    if(!options.skipPuppeteerCheck && !await hasPuppeteer()) throw new Error("ERR_NO_DEP: Puppeteer not found")
    if(options.embeddedVideoUrl && !options.embeddedVideoUrl.startsWith("https://www.youtube.com/embed/")) throw new Error("ERR_INVALID_YT_EMBED: That is not a valid youtube embed")
    ...
    return new Promise<GeneratorReturnData>(async (resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            ...
            reject("ERR_PUPPETEER_TIMEOUT: Timeout exceeded. Use GeneratorOptions.timeout to increase it")
        ...
        client.on("Network.requestWillBeSent", (e) => {
            if(e.request.url.includes("/youtubei/v1/player")) {
                ...
                if(!jsonData["serviceIntegrityDimensions"]["poToken"] || !jsonData["context"]["client"]["visitorData"]) reject("Unable to get poToken or visitorData")
        ...
        await page.goto(options.embeddedVideoUrl ?? "https://www.youtube.com/embed/jNQXAC9IVRw", {
