...
export type LiveChatEvents = typeof LiveChatEvents[keyof typeof LiveChatEvents]
...
export class LiveChat {
        ...
        if(action.is(YTNodes.AddChatItemAction)) {
            ...
            switch(item.type) {
                case "LiveChatTextMessage": {
                    this.#eventEmitter.emit(LiveChatEvents.MessageCreate, new LiveChatMessage(item, ChatMessageType.Regular))
                    break;
                }
                case "LiveChatPaidMessage": {
                    this.#eventEmitter.emit(LiveChatEvents.MessageCreate, new LiveChatMessage(item, ChatMessageType.Premium))
                    break;
                }
                case "LiveChatPaidSticker": {
                    this.#eventEmitter.emit(LiveChatEvents.MessageCreate, new LiveChatMessage(item, ChatMessageType.PremiumSticker))
                    break;
                }
                default: {
                    // noop
    ...
    destroy() {
        if(this.#hasListener[LiveChatEvents.MessageCreate]) this.chat.off("chat-update", this.chatUpdateHandler)
        if(this.#hasListener[LiveChatEvents.StreamEnd]) this.chat.off("end", this.#chatEndHandler)
